...
import com.rabbitminers.druidry.base.helpers.AngleHelper;
import com.rabbitminers.druidry.base.helpers.Colour;
...
public abstract class Outline {
    ...
    public abstract void render(PoseStack ms, TriBufferSource buffer, float pt);
    ...
    public void bufferCuboidLine(PoseStack poseStack, VertexConsumer consumer, Vector3f start, Vector3f end, float width, Vector4f color, int lightmap, boolean disableNormals) {
        ...
        float hAngle = AngleHelper.degreesFromDouble(Mth.atan2(diff.x(), diff.z()));
        float hDistance = Mth.sqrt(diff.x() * diff.x() + diff.z() * diff.z());
        float vAngle = AngleHelper.degreesFromDouble(Mth.atan2(hDistance, diff.y())) - 90;
    ...
    public static class OutlineParams {
        protected Optional<DruidryTextures> faceTexture;
        protected Optional<DruidryTextures> hightlightedFaceTexture;
        ...
        public OutlineParams() {
            ...
            lineWidth = 1 / 32f;
            fadeLineWidth = true;
            rgb = new Colour(255, 255, 255);
            lightmap = LightTexture.FULL_BRIGHT;
        }

        // builder

        public OutlineParams colored(int colour) {
            rgb = new Colour(colour);
            return this;
        }

        public OutlineParams colored(Colour c) {
            rgb = c.copy();
            return this;
        }
        ...
        public OutlineParams withFaceTexture(DruidryTextures texture) {
        ...
        public OutlineParams withFaceTextures(DruidryTextures texture, DruidryTextures highlightTexture) {
            this.faceTexture = Optional.ofNullable(texture);
            this.hightlightedFaceTexture = Optional.ofNullable(highlightTexture);
            return this;
        }

        public OutlineParams highlightFace(@NotNull Direction face) {
        ...
        public void loadColor(Vector4f vec) {
            vec.set(rgb.getRed(), rgb.getGreen(), rgb.getBlue(), rgb.getAlpha() * alpha);
        }
    }
