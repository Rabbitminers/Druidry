...
public class AABBOutline extends Outline {
    ...
    @Override
    public void render(PoseStack ms, TriBufferSource buffer, float pt) {
    ...
    protected void renderBox(PoseStack ms, TriBufferSource buffer, AABB box, Vector4f color, int lightmap, boolean disableLineNormals) {
        ...
        VertexConsumer consumer = buffer.getBuffer(CustomRenderTypes.getOutlineSolid());
        renderBoxEdges(ms, consumer, minPos, maxPos, lineWidth, color, lightmap, disableLineNormals);
    }

    protected void renderBoxFaces(PoseStack ms, TriBufferSource buffer, boolean cull, Direction highlightedFace, Vector3f minPos, Vector3f maxPos, Vector4f color, int lightmap) {
    ...
    protected void renderBoxFace(PoseStack.Pose pose, TriBufferSource buffer, boolean cull, Direction highlightedFace, Vector3f minPos, Vector3f maxPos, Direction face, Vector4f color, int lightmap) {
        boolean highlighted = face == highlightedFace;

        Optional<DruidryTextures> optionalFaceTexture = params.faceTexture;
        if (optionalFaceTexture.isEmpty())
            return;
        DruidryTextures faceTexture = optionalFaceTexture.get();

        RenderType renderType = CustomRenderTypes.getOutlineTranslucent(faceTexture.getLocation(), cull);
